...
- ONLY output the interviewer's words. NEVER write candidate responses."""
    ...
    def next_question(self, history: list, job_role: str, difficulty: str,
                      company: str = None, emotion: str = "neutral",
                      answer_eval: dict = None) -> str:
        """
        Behaves like a real human interviewer.

        Decision matrix based on answer_eval score:
          0-4  (Poor/Below Avg) : Gently probe — acknowledge + targeted follow-up
          5-6  (Average)        : Light probe  — acknowledge + ask for one example
          7-10 (Good/Excellent) : Move forward — briefly acknowledge + new question
          < 15 words (any score): Treat as incomplete regardless of score

        Also adapts tone based on detected emotion and winds down after q10+.
        """
        ...
        score     = answer_eval.get("score", 7)    if answer_eval else 7
        missing   = answer_eval.get("what_was_missing", []) if answer_eval else []
        word_count = len(last_candidate.split())
        is_very_short = 0 < word_count < 15

        if is_very_short or score <= 4:
            answer_directive = (
                "SITUATION — INCOMPLETE OR VERY WEAK ANSWER:\n"
                "The candidate gave a very short or incomplete answer. "
                "A good human interviewer does NOT just move on. You must:\n"
                "1. In one brief, warm sentence acknowledge what they did say\n"
                "2. Ask a gentle, specific follow-up to help them elaborate — "
                "for example: 'Could you walk me through a specific example?' or "
                "'Can you expand a bit on how you approached that?'\n"
                f"Key gaps to address: {chr(10).join(f'- {g}' for g in missing[:2]) if missing else '- depth and concrete specifics'}\n"
                "Keep your tone supportive and non-judgmental. Never make them feel cornered."
            )
        elif score <= 6:
            answer_directive = (
                "SITUATION — SURFACE-LEVEL ANSWER:\n"
                "The candidate gave a reasonable but shallow answer — it covered the basics "
                "but lacked depth or specific examples. As a human interviewer:\n"
                "1. Briefly acknowledge positively (one sentence)\n"
                "2. Ask ONE targeted follow-up to draw out more substance — "
                "e.g. 'That's a good point — could you give me a specific example of that?' or "
                "'How did that play out in practice?'\n"
                f"What to probe: {chr(10).join(f'- {g}' for g in missing[:2]) if missing else '- concrete examples or measurable outcomes'}\n"
                "After they answer this follow-up, move to a fresh topic."
            )
        else:
            answer_directive = (
                "SITUATION — STRONG ANSWER:\n"
                "The candidate gave a good or excellent answer. Briefly acknowledge it "
                "(one natural sentence) and move the interview forward with a new question "
                "on a different topic or a deeper dimension of the same area. "
                "You may slightly increase difficulty if the candidate seems confident."
            )

        # ── Emotion-based tone ────────────────────────────────────────────────
        tone_hint = ""
        if emotion in ["nervous", "sad"]:
            tone_hint = "Tone note: The candidate seems nervous or stressed — keep your voice warm, calm, and encouraging throughout."
        elif emotion == "disengaged":
            tone_hint = "Tone note: The candidate seems disengaged — ask something more direct and genuinely interesting to re-engage them."
        elif emotion == "enthusiastic":
            tone_hint = "Tone note: The candidate is enthusiastic — match their energy and build on it naturally."

        # ── Wind-down ─────────────────────────────────────────────────────────
        winding_down = ""
        if q_count >= 10:
            winding_down = (
                "Note: You have asked 10+ questions. After this exchange, "
                "invite the candidate to ask you questions and begin wrapping up the interview."
            )

        ctx = self.kb.context(company) if company and company != "General" else ""
        sys = f"""You are a skilled, empathetic human interviewer conducting a {difficulty} {job_role} interview{f' at {company}' if company and company != "General" else ''}.
{ctx}

Recent conversation (last 10 turns):
{recent}

Total interviewer turns so far: {q_count}
...
STRICT RULES:
- Output ONLY the interviewer's words. NEVER write the candidate's response.
- Be conversational and natural — like a thoughtful human, not a script.
- Maximum 2-3 sentences. Do not lecture or over-explain.
- Do NOT repeat any question already asked.
- Always reference something from what the candidate just said — show you were listening."""

        return self._gpt(sys, "Generate the interviewer's next response only.")
...
{{
  ...
  "actionable_recommendations": []
    ...
    def evaluate_answer(self, question: str, answer: str,
                        job_role: str, company: str = None) -> dict:
        """
        Score how well the candidate answered a specific question.
        Returns a dict with: score, score_label, what_you_did_well,
        what_was_missing, ideal_answer.
        Called live after every answer, stored in conv[].
        """
        if not question.strip() or not answer.strip():
            return {}
        company_ctx = f" at {company}" if company and company != "General" else ""
        sys = f"""You are a senior interviewer evaluating a candidate's answer for a {job_role} role{company_ctx}.
...
{{
  ...
  "ideal_answer": "<A concise 3-5 sentence model answer that would score 10/10 for this specific question and role>"
}}

Scoring guide:
0-3  Poor          — irrelevant, very incomplete, or wrong
4-5  Below Average — partially correct but major gaps
6-7  Average       — covers basics but lacks depth or examples
8-9  Good          — clear, structured, with good examples
10   Excellent     — complete, specific, well-structured, impressive

Be honest and specific. ideal_answer must directly answer the question asked."""

        try:
            raw = self.client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[{"role":"system","content":sys},
                          {"role":"user","content":"Return the evaluation JSON only."}],
...
def main():
        ...
        if st.session_state.active:
            ...
            if audio_input is not None:
                ...
                with st.spinner("🎙️ Transcribing… 🧠 Analysing tone…"):
                    ...
                    # ── Step 2.5: Answer Quality Evaluation ───────────────────
                    last_q_for_eval = next((m["content"] for m in reversed(st.session_state.conv)
                                            if m["role"] == "interviewer"), "")
                    answer_eval = {}
                    if last_q_for_eval and transcript and not transcript.startswith("["):
                        try:
                            answer_eval = iw.evaluate_answer(
                                last_q_for_eval, transcript, job_role, company)
                        except Exception:
                            answer_eval = {}
                    ...
                    st.session_state.conv.append({
                        ...
                        "content":     transcript,
                        "tone":        tone_result,
                        "answer_eval": answer_eval,   # ← new: per-question score
                    })
                    ...
                    next_q = iw.next_question(
                        ...
                        job_role, difficulty, company,
                        emotion=st.session_state.last_emotion,
                        answer_eval=answer_eval   # pass quality eval for smart probing
                    )
    ...
    if st.session_state.feedback and not st.session_state.active:
        ...
        # ── SECTION 3: PER-QUESTION ANSWER REVIEW ────────────────────────────
        qa_pairs = [m for m in st.session_state.conv
                    if m["role"] == "candidate" and m.get("answer_eval")]
        if qa_pairs:
            st.markdown("## 📝 Per-Question Answer Review")
            st.caption("Score and model answer for every question you were asked")

            # Summary score bar across all questions
            all_scores = [m["answer_eval"].get("score", 0) for m in qa_pairs
                          if isinstance(m["answer_eval"].get("score"), (int, float))]
            if all_scores:
                avg_ans = round(sum(all_scores) / len(all_scores), 1)
                GRADE_COLOR = {
                    "Excellent":     "#4caf89",
                    "Good":          "#80c7b7",
                    "Average":       "#f5c842",
                    "Below Average": "#e8935a",
                    "Poor":          "#e05c5c",
                }

                # Mini scorecard row
                sc_cols = st.columns(min(len(all_scores), 10))
                for idx, m in enumerate(qa_pairs):
                    ev  = m["answer_eval"]
                    scr = ev.get("score", 0)
                    lbl = ev.get("score_label", "")
                    col = GRADE_COLOR.get(lbl, "#888")
                    with sc_cols[idx % len(sc_cols)]:
                        st.markdown(f"""
                        <div style='text-align:center;padding:10px 6px;
                                    background:{col}18;border:1px solid {col}44;
                                    border-radius:10px;'>
                            <div style='font-size:1.4rem;font-weight:900;color:{col};'>{scr}</div>
                            <div style='font-size:0.62rem;color:{col};font-weight:700;'>/10</div>
                            <div style='font-size:0.6rem;color:#555;'>Q#{idx+1}</div>
                        </div>""", unsafe_allow_html=True)

                st.markdown(f"""
                <div style='margin:16px 0 8px;padding:12px 18px;
                    ...
                    <div>
                        <span style='font-size:0.72rem;color:#666;
                            ...
                            Avg Answer Score</span><br>
                        <span style='font-size:2rem;font-weight:900;
                                     color:{"#4caf89" if avg_ans>=7 else "#f5c842" if avg_ans>=5 else "#e8935a"};'>
                            {avg_ans}<span style='font-size:1rem;color:#555;'>/10</span>
                        </span>
                    </div>
                    <div style='flex:1;'>
                        <div style='height:8px;background:rgba(255,255,255,0.06);
                                    border-radius:4px;overflow:hidden;'>
                            <div style='height:100%;width:{avg_ans*10}%;
                                        background:{"#4caf89" if avg_ans>=7 else "#f5c842" if avg_ans>=5 else "#e8935a"};
            ...
            for idx, m in enumerate(qa_pairs):
                ...
                if not ev or "error" in ev:
                    continue
                scr   = ev.get("score", 0)
                lbl   = ev.get("score_label", "N/A")
                well  = ev.get("what_you_did_well", [])
                miss  = ev.get("what_was_missing", [])
                ideal = ev.get("ideal_answer", "")
                col   = GRADE_COLOR.get(lbl, "#888")

                # Reconstruct the question from conv (the message before this candidate msg)
                conv_list = st.session_state.conv
                cand_idx  = conv_list.index(m)
                q_text = next(
                    (conv_list[j]["content"] for j in range(cand_idx-1, -1, -1)
                     if conv_list[j]["role"] == "interviewer"), "")

                with st.expander(
                    f"Q#{idx+1} — Score: {scr}/10  [{lbl}]  "
                    f"{'★'*scr}{'☆'*(10-scr)}"
                ):
                    # Question + Answer
                    st.markdown(f"""
                    <div style='background:rgba(255,255,255,0.03);border-left:3px solid #4caf89;
                                border-radius:0 8px 8px 0;padding:10px 14px;margin-bottom:10px;'>
                        <div style='font-size:0.68rem;color:#4caf89;font-weight:700;
                                    text-transform:uppercase;letter-spacing:0.1em;'>Question</div>
                        <div style='color:#ddd;margin-top:4px;'>{q_text}</div>
                    </div>
                    <div style='background:rgba(255,255,255,0.03);border-left:3px solid #5b8cdb;
                                border-radius:0 8px 8px 0;padding:10px 14px;margin-bottom:14px;'>
                        <div style='font-size:0.68rem;color:#5b8cdb;font-weight:700;
                                    text-transform:uppercase;letter-spacing:0.1em;'>Your Answer</div>
                        <div style='color:#bbb;margin-top:4px;font-style:italic;'>
                            {m["content"]}</div>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    with g1:
                        ...
                        gauge = go.Figure(go.Indicator(
                            ...
                            value=scr,
                            domain={"x":[0,1],"y":[0,1]},
                            title={"text": lbl, "font": {"size":11,"color":"#aaa"}},
                            gauge={
                                ...
                                "steps": [
                                    {"range":[0,3],  "color":"rgba(224,92,92,0.15)"},
                                    {"range":[3,5],  "color":"rgba(232,147,90,0.15)"},
                                    {"range":[5,7],  "color":"rgba(245,200,66,0.15)"},
                                    {"range":[7,9],  "color":"rgba(128,199,183,0.15)"},
                                    {"range":[9,10], "color":"rgba(76,175,137,0.15)"},
                                ],
                                "threshold": {
                                    "line":{"color":col,"width":3},
                                    "thickness":0.85, "value": scr
                                }
                            },
                            number={"font":{"color":col,"size":28},"suffix":"/10"},
                        ))
                        gauge.update_layout(
                    ...
                    with g2:
                        ...
                        if well:
                            for pt in well:
                                st.markdown(f"""
                                <div style='background:rgba(76,175,137,0.08);
                                            border-left:2px solid #4caf89;
                    ...
                    with g3:
                        ...
                        if miss:
                            for pt in miss:
                                st.markdown(f"""
                                <div style='background:rgba(232,147,90,0.08);
                                            border-left:2px solid #e8935a;
                                            border-radius:0 6px 6px 0;
                                            padding:6px 10px;margin:4px 0;
                                            font-size:0.85rem;color:#ccc;'>
                                    {pt}
                            ...
                            st.caption("Nothing major missing.")

                    # Ideal answer
                    if ideal:
                        st.markdown(f"""
                        <div style='margin-top:14px;background:rgba(245,200,66,0.06);
                                    border:1px solid rgba(245,200,66,0.25);
                                    border-radius:10px;padding:14px 16px;'>
                            <div style='font-size:0.7rem;color:#f5c842;font-weight:700;
                                        text-transform:uppercase;letter-spacing:0.1em;
                                        margin-bottom:8px;'>
                                🏆 Model Answer (10/10)
                            </div>
        ...
        # ── SECTION 2 (renumbered): INTERVIEW QUALITY REPORT ─────────────────
        st.markdown("## 🏆 Interview Quality Report")
        ...
        # Build per-question answer review text
        qa_txt = ""
        qa_pairs_dl = [m for m in st.session_state.conv
                       if m["role"] == "candidate" and m.get("answer_eval")]
        if qa_pairs_dl:
            qa_txt = "\n\nPER-QUESTION ANSWER REVIEW\n" + "="*55 + "\n"
            conv_list = st.session_state.conv
            for idx, m in enumerate(qa_pairs_dl, 1):
                ev = m.get("answer_eval", {})
                if not ev or "error" in ev:
                    continue
                cand_idx = conv_list.index(m)
                q_text   = next(
                    (conv_list[j]["content"] for j in range(cand_idx-1, -1, -1)
                     if conv_list[j]["role"] == "interviewer"), "N/A")
                qa_txt += (
                    f"\nQ#{idx}: {q_text}\n"
                    f"Your Answer : {m['content']}\n"
                    f"Score       : {ev.get('score','?')}/10  [{ev.get('score_label','')}]\n"
                    f"Did well    : {'; '.join(ev.get('what_you_did_well',[]))}\n"
                    f"Missing     : {'; '.join(ev.get('what_was_missing',[]))}\n"
                    f"Ideal Answer: {ev.get('ideal_answer','')}\n"
                    f"{'─'*50}\n"
                )
